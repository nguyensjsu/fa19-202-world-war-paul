    ...
    public void touch(int x, int y)
    {
      ...
      else if(x > 0 && x < 380 && y > 620 && y < 680)
      {
        if(cardNumber.length() == 16 && cardExpirty.length() == 4 && cvv.length() == 3)
        {
          ...
          JSONObject info = new JSONObject(cardInfo);
      ...
      if((cardNumberFlag && cardNumber.length() < 16) || (cardNumberFlag 
                            && y >= 530 && y <= 590 && x >= 265 && x <= 325))
      {
        inputTouch(cardNumber, x, y, "cardNumber");
      }
      else if((cardExpirtyFlag && cardExpirty.length() < 4) || (cardExpirtyFlag 
                            && y >= 530 && y <= 590 && x >= 265 && x <= 325))
      {
